 ...
 **/
/* Optional hook: mirror printf output to the USB debug CDC port. Provided by
 * debug_cdc.c when DEBUG_OVER_USB is enabled; weak so the SDK still links
 * standalone. */
__attribute__((weak)) void debug_mirror(const char *buf, int size) { (void)buf; (void)size; }

__attribute__((used)) int _write(int fd, char *buf, int size)
{
    int i;

    debug_mirror(buf, size);
